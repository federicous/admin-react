...
export default function BasicExampleDataGrid() {
	...
	const columnsBloqueados = [
		{ field: 'code', headerName: 'Código Factura', width: 130, flex:0.8, minWidth:110 },
		{ field: 'proveedor', headerName: 'Proveedor', width: 230, flex:0.8 , minWidth:110},
		...
		{ field: 'medioDePago', headerName: 'Medio de pago', width: 130, flex:0.8, minWidth:110 },
		];
	...
	// 		// setOpen(true);
	// 		apiQuery.get(`/api/permisos/datos`).then((respuesta) => {
	// 			if (cancel) return;
	// 			setIsAdmin(respuesta?.isAdmin == "on" ? true : false);
	// 			setUserPermissions(
	// 				respuesta?.permisos?.length ? respuesta.permisos : []
	// 			);
	// 			setUserAccesos(
	// 				respuesta?.accesos?.length ? respuesta.accesos : []
	// 			);
	// 			// setOpen(false);
	// 		});
	// 	}
	...
	function removeElement(id) {
		setOpen(true);
		let cancel = false;
		apiQuery.delete(`/api/pagos/${id}`).then((response) => {
			if (cancel) return;
	...
	<Grid container spacing={3}>		
		<Grid item xs={12} sm={12}>
				...
				<Box sx={{ height: 600, width: '100%' }}>
					<DataGrid rows={listaBloqueados} columns={(isAdmin || userAccesos.includes("desbloqueoSender")) ? columnsBloqueadosAdmin : columnsBloqueados} getRowId={(row) => row._id} 
