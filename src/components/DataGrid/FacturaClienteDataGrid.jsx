...
export default function BasicExampleDataGrid() {
	...
	const columnsBloqueados = [
		...
		{ field: 'vencimiento', headerName: 'Vencimiento', width: 130, flex:0.8, minWidth:110 },
	...
	// 		// setOpen(true);
	// 		apiQuery.get(`/api/permisos`).then((respuesta) => {
	// 			if (cancel) return;
	...
	function removeElement(id) {
		setOpen(true);
		let cancel = false;
		apiQuery.delete(`/api/facturaCliente/${id}`).then((response) => {
			if (cancel) return;
